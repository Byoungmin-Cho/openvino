...
#else
    #define TRIANGLE_COEFF(a, x) (ACCUMULATOR_MAX_FUNC(ACCUMULATOR_VAL_ZERO, ACCUMULATOR_VAL_ONE - ACCUMULATOR_ABS_FUNC(a * x)))
#endif
...
)
{
    ...
    const int ox = xy % OUTPUT_SIZE_X;
    const int oy = xy / OUTPUT_SIZE_X;
    ...
    // if (get_group_id(0) == 0 && get_group_id(1) < 2)
    //     printf("gid:%d b:%d f:%d y:%d x:%d xy:(%d) %d %d %d\n",get_group_id(0), batch, feature, oy, ox, xy, get_sub_group_size(), get_sub_group_id(), get_sub_group_local_id());
    ...
    unroll_for(int b = b_init; b < b_max; b++) {
        ...
        unroll_for(int f = f_init; f < f_max; f++) {
            wf = TRIANGLE_COEFF(af, i_f - f);
            unroll_for(int y = y_init; y < y_max; y++) {
                wy = TRIANGLE_COEFF(ay, i_y - y);
    ...
    const int xy = get_global_id(1);
    const int b = get_global_id(2);
    const int f_block = get_group_id(0);
